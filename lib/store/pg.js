...
const COLS = 'id, name, author, piece_count, lanes, length_cm, bbox_w_cm, bbox_h_cm, straights, corners, stars, complete, issues, validator_version, created_at, updated_at';
...
export async function open(connectionString) {
  ...
  for (const col of ['straights INTEGER NOT NULL DEFAULT 0', 'corners INTEGER NOT NULL DEFAULT 0',
    ...
    await pool.query(`ALTER TABLE tracks ADD COLUMN ${col}`).catch(() => {});
  }
    ...
    async upsert(t) {
      ...
      const res = await pool.query(`
        INSERT INTO tracks (id, name, author, data, piece_count, lanes, length_cm, bbox_w_cm, bbox_h_cm,
                            straights, corners, complete, issues, validator_version, created_at, updated_at)
        ...
        [t.id, t.name, t.author ?? null, JSON.stringify(t.data),
         ...
         f.straights, f.corners, f.complete ? 1 : 0, f.issues ?? 0, f.validator_version ?? 0, ts, ts]);
