...
export async function open(connectionString) {
  ...
  /* per-track archive serialization (see archive below, worklog 0022) */
  const archiving = new Map();   /* track id -> completion marker of the latest archive */
  ...
  return {
     ...
     * INSERT→SELECT→DELETE is a read-modify-write, so same-track archives
     * are serialized (a concurrent prune must not erase a just-inserted
     * snapshot it never saw). */
    async archive(id, snapshot) {
      const prev = (archiving.get(id) ?? Promise.resolve()).catch(() => {});
      let done;
      const tail = new Promise((r) => { done = r; });   /* never rejects: one failure can't poison the chain */
      archiving.set(id, tail);
      try {
        await prev;   /* queue behind same-track archives */
        ...
        return keep.size;
      } finally { done(); if (archiving.get(id) === tail) archiving.delete(id); }
    },
    async history(id) {
