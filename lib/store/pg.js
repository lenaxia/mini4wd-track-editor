...
export async function open(connectionString) {
  ...
  return {
    ...
    async upsert(t) {
      ...
      const res = await pool.query(`
        ...
        [t.id, t.name, t.author ?? null, JSON.stringify(t.data),
         ...
         f.straights, f.corners, f.complete ? 1 : 0, f.issues ?? 0, f.validator_version ?? 0,
      ...
      return { ...m, created_at: Number(m.created_at), updated_at: Number(m.updated_at),
               ...
               straights: Number(m.straights), corners: Number(m.corners) };
    },
    async star(id) {
    ...
    /* Version history — worklog 0017. Same contract as sqlite. */
    async archive(id, snapshot) {
      ...
      await pool.query(`
        DELETE FROM revisions WHERE track_id = $1 AND seq NOT IN (
          SELECT seq FROM revisions WHERE track_id = $1 ORDER BY seq DESC LIMIT $2
        )`, [id, REVISION_CAP]);
      const n = await pool.query('SELECT COUNT(*) c FROM revisions WHERE track_id = $1', [id]);
