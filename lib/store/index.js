...
export async function openStore(env = process.env) {
  ...
  if (kind === 'sqlite') {
    ...
    /* absent dir: bounded async mkdir — sync recursive mkdir can spin
     * forever on ENOENT-lying mounts (issue #66 follow-up) */
    if (!fs.existsSync(path.dirname(p)))
      await mkdirBounded(path.dirname(p), { label: `SQLITE_PATH=${p}` });
