...
const COLS = 'id, name, author, piece_count, lanes, length_cm, bbox_w_cm, bbox_h_cm, straights, corners, stars, complete, issues, validator_version, parent_id, root_id, parent_name, created_at, updated_at';
...
export function open(path) {
  ...
  for (const col of ['straights INTEGER NOT NULL DEFAULT 0', 'corners INTEGER NOT NULL DEFAULT 0',
  ...
  return {
    ...
    async upsert(t) {
      ...
      return db.prepare(`
        INSERT INTO tracks (id, name, author, data, piece_count, lanes, length_cm, bbox_w_cm, bbox_h_cm,
                            straights, corners, complete, issues, validator_version, parent_id, root_id, parent_name, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT (id) DO UPDATE SET
          ...
          corners = excluded.corners, complete = excluded.complete,
        ...
        f.bbox_w_cm, f.bbox_h_cm, f.straights, f.corners,
    ...
    /* Version history — worklog 0017. Archive keeps the newest
     * REVISION_CAP snapshots per track (prune with a subselect). */
    async archive(id, snapshot) {
      ...
      db.prepare(`
        DELETE FROM revisions WHERE track_id = ? AND seq NOT IN (
          SELECT seq FROM revisions WHERE track_id = ? ORDER BY seq DESC LIMIT ?
        )`).run(id, id, REVISION_CAP);
      return db.prepare('SELECT COUNT(*) c FROM revisions WHERE track_id = ?').get(id).c;
    },
    async history(id) {
