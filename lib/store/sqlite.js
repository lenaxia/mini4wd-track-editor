...
const COLS = 'id, name, author, piece_count, lanes, length_cm, bbox_w_cm, bbox_h_cm, straights, corners, slopes, stars, complete, issues, validator_version, parent_id, root_id, parent_name, author_trip, created_at, updated_at';
...
export function open(path) {
  ...
  for (const col of ['straights INTEGER NOT NULL DEFAULT 0', 'corners INTEGER NOT NULL DEFAULT 0', 'slopes INTEGER NOT NULL DEFAULT 0',
                     ...
                     'parent_id TEXT', 'root_id TEXT', 'parent_name TEXT', 'author_trip TEXT']) {
  ...
  return {
    ...
    async upsert(t) {
      ...
      return db.prepare(`
        INSERT INTO tracks (id, name, author, data, piece_count, lanes, length_cm, bbox_w_cm, bbox_h_cm,
                            straights, corners, slopes, complete, issues, validator_version, parent_id, root_id, parent_name, author_trip, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT (id) DO UPDATE SET
          ...
          author_trip = excluded.author_trip,
        ...
        f.complete ? 1 : 0, f.issues ?? 0, f.validator_version ?? 0,
        t.parent_id ?? null, t.root_id ?? null, t.parent_name ?? null, t.author_trip ?? null, ts, ts);
    },
    async star(id) {
    ...
    async remove(id) {
      db.prepare('DELETE FROM revisions WHERE track_id = ?').run(id);
      const r = db.prepare('DELETE FROM tracks WHERE id = ?').run(id);
      return r.changes > 0;
    },
    /* Version history — worklogs 0020/0022. Every insert prunes to the
     * tier ladder via keepSet; REVISION_CAP remains the backstop. */
    async archive(id, snapshot) {
      db.prepare('INSERT INTO revisions (track_id, created_at, snapshot) VALUES (?, ?, ?)')
        .run(id, Date.now(), JSON.stringify(snapshot));
      const rows = db.prepare('SELECT seq, created_at FROM revisions WHERE track_id = ? ORDER BY seq DESC').all(id);
      const keep = keepSet(rows);
      if (rows.length > REVISION_CAP) for (const r of rows.slice(REVISION_CAP)) keep.delete(r.seq);
      const keepSeqs = [...keep];
      if (keepSeqs.length < rows.length) {
        db.prepare(`DELETE FROM revisions WHERE track_id = ? AND seq NOT IN (${keepSeqs.map(() => '?').join(',')})`)
          .run(id, ...keepSeqs);
      }
      return keepSeqs.length;
    },
    async history(id) {
      return db.prepare('SELECT seq, created_at, snapshot FROM revisions WHERE track_id = ? ORDER BY seq DESC')
        .all(id).map(({ seq, created_at, snapshot }) => ({ seq, created_at, name: JSON.parse(snapshot).name }));
    },
    async revision(id, seq) {
      const r = db.prepare('SELECT snapshot FROM revisions WHERE track_id = ? AND seq = ?').get(id, seq);
