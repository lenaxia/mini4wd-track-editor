...
import { mkdirBounded } from './mkdir-bounded.js';

let cachedSalt = null;
let saltSrc = null;

/* Test seam: drop the memoized salt so a fresh env re-resolves. */
export function _resetSaltCache() { cachedSalt = null; saltSrc = null; }

/* Where the live salt came from: 'env' | 'file' | 'ephemeral'
 * (null before the first resolution) — lets the server log a warning
 * when locks silently became non-persistent. */
export function saltSource() { return saltSrc; }

/* Boot-time entry (server.js): resolves the salt once, up front. The
 * only unbounded-IO hazard (recursive mkdir on an ENOENT-lying mount)
 * is raced in mkdirBounded and kills the boot loudly naming the path;
 * honest fs errors degrade to a logged ephemeral salt. */
export async function initSalt(env = process.env) {
  if (env.M4WD_TRIP_SALT) { saltSrc = 'env'; return env.M4WD_TRIP_SALT; }
  if (cachedSalt) return cachedSalt;
  const dir = path.dirname(env.SQLITE_PATH || path.join('data', 'tracks.db'));
  try {
    if (!fs.existsSync(dir))
      await mkdirBounded(dir, { label: `salt dir for SQLITE_PATH=${env.SQLITE_PATH || 'data/tracks.db'}` });
    ...
    if (e && e.code === 'M4WD_MKDIR_TIMEOUT') throw e;   /* loud boot death, never a silent degrade */
    saltSrc = 'ephemeral';
    return cachedSalt = crypto.randomBytes(16).toString('hex');
  }
}

/* Lazy fallback (server.js always awaits initSalt() first, so this
 * only serves fresh-cache callers): reads or writes the salt when the
 * dir is already there; it never CREATES directories — sync recursive
 * mkdir can spin forever on ENOENT-lying mounts, so an absent dir
 * degrades to ephemeral instead. */
function getSalt(env = process.env) {
   ...
   * SQLITE_PATH falls back to ./data under the CWD) */
  const dir = path.dirname(env.SQLITE_PATH || path.join('data', 'tracks.db'));
  try {
    const p = path.join(dir, 'trip.salt');
    if (fs.existsSync(p)) { saltSrc = 'file'; return cachedSalt = fs.readFileSync(p, 'utf8').trim(); }
    if (!fs.existsSync(dir)) { saltSrc = 'ephemeral'; return cachedSalt = crypto.randomBytes(16).toString('hex'); }
    cachedSalt = crypto.randomBytes(16).toString('hex');
    fs.writeFileSync(p, cachedSalt + '\n');
    saltSrc = 'file';
    return cachedSalt;
  } catch { saltSrc = 'ephemeral'; return cachedSalt = crypto.randomBytes(16).toString('hex'); }
}
