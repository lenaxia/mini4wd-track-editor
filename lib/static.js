...
import { acceptsGzip, compressible } from './compress.js';
...
export function isServable(urlPath) {
  /* normalize first so ../ segments cannot smuggle a denied path behind
   * an allowed prefix (/src/../server.js must not read as /src/) */
  const p = path.posix.normalize(urlPath);
  return p === '/' || p === '/index.html' || p === '/style.css' ||
         p.startsWith('/src/') || p.startsWith('/assets/');
}

export function createStaticHandler(root) {
  return (req, res) => {
    const url = decodeURIComponent(req.url.split('?')[0]);
    /* allowlist (issue #62): serve exactly the shipped app surface — the
     * document, its stylesheet, the modules and the art. Anything else
     * (data/, .git/, lib/, server code, docker-compose.yml…) is refused
     * before any fs call and 404s like a missing file. */
    if (!isServable(url)) return false;
    ...
    const headers = {
      ...
      /* hardening (issue #67), usage-derived: same-origin module script
       * and stylesheet (index.html), inline style= attributes in static
       * and generated markup (ui.js), sprites as /assets URLs and blob:
       * object URLs (cache.js), fetches only against this origin. On
       * every static response — harmless off-document, and 304s match. */
    ...
    if (path.basename(file) === 'index.html' &&
      ...
      /* SameSite=Lax per issue #67; NOT HttpOnly — the client reads and
       * clears this cookie via document.cookie (src/cache.js hard-refresh
       * wipe, pinned by tests/e2e/caching.spec.js) */
      headers['Set-Cookie'] = 'm4wd_hard=1; Max-Age=10; Path=/; SameSite=Lax';
    }
    if (req.headers['if-none-match'] === etag) { res.writeHead(304, headers); res.end(); return true; }
    /* gzip eligible types for gzip-capable clients; Vary rides on every
     * compressible response (compressed or not) so shared caches key on
     * the encoding. The ETag stays identity-based — it names the FILE. */
    const eligible = compressible(headers['Content-Type'], stat.size);
    const gz = eligible && acceptsGzip(req);
    if (eligible) headers.Vary = 'Accept-Encoding';
    if (gz) headers['Content-Encoding'] = 'gzip';
    res.writeHead(200, headers);
    const stream = fs.createReadStream(file);
    stream.on('error', () => res.destroy());   /* TOCTOU unlink/EACCES: never hang the socket */
    if (gz) stream.pipe(zlib.createGzip({ level: 6 })).pipe(res);
